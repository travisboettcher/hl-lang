...
fn unexpected_char_errors() {
    // `!`, `&` and `|` are back in this set: #228 gave them tokens for
    // the `rule` expression's operators, and #271 removed that grammar,
    // so none of the three starts a token again.
    for ch in ['@', '%', ';', '\\', '!', '&', '|'] {
